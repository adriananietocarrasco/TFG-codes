...
results_dir = "optuna3_resultsGRUBIEN_angles"
...
class SeqGRUModel(nn.Module):
    # output_dim cambiado a 1
    def __init__(self, input_dim=1, hidden_dim=50, output_dim=1, num_layers=2, dropout_prob=0.2):
...
def objective(trial):
    ...
    # Modelo basado en GRUnext2
    model = SeqGRUModel(hidden_dim=hidden_dim, num_layers=num_layers, dropout_prob=dropout_prob).to(device)
...
if hasattr(fig_contour, "figure"):
    # Es un solo eje
    fig_contour.figure.savefig(os.path.join(results_dir, "contour.png"))
elif isinstance(fig_contour, np.ndarray):
    # Es un array de ejes: guarda la figura del primer eje
    fig_contour.flat[0].figure.savefig(os.path.join(results_dir, "contour.png"))
else:
    print("No se pudo guardar el gráfico de contorno: tipo inesperado.")
