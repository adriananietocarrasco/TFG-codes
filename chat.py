...
#Este file lo que hace es obtener con 4 DOF, mas el tiempo los joints.mot para despues meterlos a open sim
...
#inverse_dynamics.py
    ...
    vec_flex1 = np.asarray(joint_flex, dtype=float)
    print("vector shape:", vec_flex1.shape)
    print("primeros valores:", vec_flex1[:10])
    ...
    vec_flex2 = np.asarray(joint_flex, dtype=float)
...
#ME HE QUEDADO AQUI DIA 17 DE NOVIEMBRE 2023
    ...
    N = len(vec_flex1)                      # number of samples
    time = np.arange(N) / fs          # time array (0, 1/6, 2/6, ...)
            # ---- FILTRAR: quedarnos solo con el TEMBLOR (4–12 Hz) ----
    fs = float(frame_rate) # Hz (60)
    #con band pass
    flex_tremor1 = bandpass_tremor(vec_flex1, fs)#joint_flex for wrist
    flex_tremor2 = bandpass_tremor(vec_flex2, fs)#joint_flex for elbow
    dev_tremor  = bandpass_tremor(vec_dev,  fs)#joint_dev
    sup_tremor  = bandpass_tremor(vec_sup,  fs)#joint_sup
    #con butter
    flex_tremor_bc1 = bandpass_centered(vec_flex1, fs)#joint_flex
    flex_tremor_bc2 = bandpass_centered(vec_flex2, fs)#joint_flex for elbow
    dev_tremor_bc = bandpass_centered(vec_dev,  fs)#joint_dev
    sup_tremor1_bc= bandpass_centered(vec_sup,  fs)#joint_sup
    ...
    data_out = np.column_stack((time, flex_tremor_bc2, sup_tremor1_bc, flex_tremor_bc1, dev_tremor_bc))
    ...
    out_mot = os.path.join(
        output_dir,
        f'joint_{idx}_{mvn.JOINTS[idx]}_flexion_ASS-POST-FLEX_run_01_7Hzfiltro_new.mot'
    )
    ...
    header_lines = [
        ...
        "endheader",
        "time  elbow_flexion pro_sup flexion deviation"
