#FILE PARA EXTRAER LOS JOINT ANGLES SIN FILTRAR
import os
import argparse
import numpy as np
...
def main(file_name):
    ...
    for idx in [9, 13]:
        ...
        time = np.arange(N) / fs 
        data_out = np.column_stack((time, vec))  # crea una matriz con dos columnas: tiempo y joint angle
        ...
        out_mot = os.path.join(
            output_dir,
            f'joint_{idx}_{mvn.JOINTS[idx]}_flexion_ASS-POST-FLEX_run_01_nofiltro.mot'
        )
